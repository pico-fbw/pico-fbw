 ...
 * @note The Callback function must have the signature `i32 (*)(u32)`, aka it must return an `i32` and have a `u32` as a parameter.
 * Returning 0 will not reschedule the callback, returning a positive value will reschedule the callback in that many milliseconds,
 * and returning a negative value will reschedule the callback that many milleseconds from when the callback was originally scheduled.
*/
...
*/
inline void sleep_ms_blocking(u32 ms) {
...
*/
inline Timestamp timestamp_now() {
 ...
 * @return the Timestamp
*/
inline Timestamp timestamp_in_us(u64 us) {
 ...
 * @return the Timestamp
*/
inline Timestamp timestamp_in_ms(u32 ms) {
...
*/
inline u64 time_since_us(const Timestamp *timestamp) {
...
*/
inline u32 time_since_ms(const Timestamp *timestamp) {
...
*/
inline bool timestamp_reached(const Timestamp *timestamp) {
    return time_us() >= timestamp->us;
}
