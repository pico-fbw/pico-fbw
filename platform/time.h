 ...
 */
static inline void sleep_ms_blocking(u32 ms) { sleep_us_blocking(ms * 1000); }

/**
 * Creates a Timestamp representing the current time.
 * @return the Timestamp
 */
static inline Timestamp timestamp_now() { return (Timestamp){.us = time_us()}; }
 ...
 * @return the Timestamp
 */
static inline Timestamp timestamp_in_us(u64 us) { return (Timestamp){.us = time_us() + us}; }
 ...
 * @return the Timestamp
 */
static inline Timestamp timestamp_in_ms(u32 ms) { return timestamp_in_us(ms * 1000); }

/**
 * @param timestamp the Timestamp to check
 * @return the time since the Timestamp was created, in microseconds
 */
static inline u64 time_since_us(const Timestamp *timestamp) { return time_us() - timestamp->us; }
/**
 * @param timestamp the Timestamp to check
 * @return the time since the Timestamp was created, in milliseconds
 */
static inline u32 time_since_ms(const Timestamp *timestamp) { return time_since_us(timestamp) / 1000; }

/**
 * @param timestamp the Timestamp to check
 * @return true if the current time is greater than or equal to the Timestamp
 */
static inline bool timestamp_reached(const Timestamp *timestamp) { return time_us() >= timestamp->us; }
