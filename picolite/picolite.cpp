...
#define _CRT_SECURE_NO_WARNINGS
#undef PICOLITE_ERROR_CANCELLED
#endif
...
static string tool_name = "picolite";
...
char PICOLITE_ERROR_msg[512];

static void __noreturn fail(int code, const char *format, ...) {
    va_list args;
    va_start(args, format);
    vsnprintf(PICOLITE_ERROR_msg, sizeof(PICOLITE_ERROR_msg), format, args);
    va_end(args);
    fail(code, string(PICOLITE_ERROR_msg));
}
    ...
    void insert(const range& r, T t) {
        if (r.to != r.from) {
            ...
            for(; f != m.end() && f->first < r.to; f++) { // loop till we can't possibly overlap
                ...
                if (r2.intersects(r)) {
                    fail(PICOLITE_ERROR_FORMAT, "Found overlapping memory ranges 0x%08x->0x%08x and 0x%08x->%08x\n",
...
auto device_selection =
    (
        ...
        + option('f', "--force").set(settings.force) % "Force a device not in BOOTSEL mode but running compatible code to reset so the command can be executed. After executing the command (unless the command itself is a 'reboot') the device will be rebooted back to application mode" +
                option('F', "--force-no-reboot").set(settings.force_no_reboot) % "Force a device not in BOOTSEL mode but running compatible code to reset so the command can be executed. After executing the command (unless the command itself is a 'reboot') the device will be left connected and accessible to picolite, but without the RPI-RP2 drive mounted"
#endif
...
struct load_command : public cmd {
    ...
    group get_cli() override {
        return (
            (
                option('n', "--no-overwrite").set(settings.load.no_overwrite) % "When writing flash data, do not overwrite an existing program in flash. If picolite cannot determine the size/presence of the program in flash, the command fails" +
                option('N', "--no-overwrite-unsafe").set(settings.load.no_overwrite_force) % "When writing flash data, do not overwrite an existing program in flash. If picolite cannot determine the size/presence of the program in flash, the load continues anyway" +
...
int parse(const int argc, char **argv) {
    ...
    auto usage=[&]() {
        ...
        if (!selected_cmd) {
            ...
            fos.hanging_indent(0);
            fos.wrap_hard();
            fos << string("Use \"picolite help ").append(selected_cmd->name()).append("\" for more info\n");
        } else {
        ...
        if (!selected_cmd) {
            ...
            fos.hanging_indent(0);
            fos.wrap_hard();
            fos << "Use \"picolite help <cmd>\" for more info\n";
        }
        fos.flush();
    };
...
uint32_t bootrom_func_lookup(memory_access& access, uint16_t tag) {
    ...
    if (magic != BOOTROM_MAGIC) {
        if (!((magic ^ BOOTROM_MAGIC)&0xffff))
            fail(PICOLITE_ERROR_INCOMPATIBLE, "Incorrect RP2040 BOOT ROM version");
        else
            fail(PICOLITE_ERROR_INCOMPATIBLE, "RP2040 BOOT ROM not found");
    }
    ...
    fail(PICOLITE_ERROR_INCOMPATIBLE, "Reboot function not found in BOOT ROM");
}
...
static void read_and_check_elf32_header(FILE *in, elf32_header& eh_out) {
    if (1 != fread(&eh_out, sizeof(eh_out), 1, in)) {
        fail(PICOLITE_ERROR_FORMAT, "'" + settings.filename +"' is not an ELF file");
    }
    if (eh_out.common.magic != ELF_MAGIC) {
        fail(PICOLITE_ERROR_FORMAT, "'" + settings.filename +"' is not an ELF file");
    }
    if (eh_out.common.version != 1 || eh_out.common.version2 != 1) {
        fail(PICOLITE_ERROR_FORMAT, "'" + settings.filename +"' has an unsupported  ELF version");
    }
    if (eh_out.common.arch_class != 1 || eh_out.common.endianness != 1) {
        fail(PICOLITE_ERROR_INCOMPATIBLE, "'" + settings.filename +"' is not a 32 bit little-endian ELF");
    }
    if (eh_out.eh_size != sizeof(struct elf32_header)) {
        fail(PICOLITE_ERROR_FORMAT, "'" + settings.filename + "' is not valid");
    }
    if (eh_out.common.machine != EM_ARM) {
        fail(PICOLITE_ERROR_FORMAT, "'" + settings.filename +"' is not an ARM executable");
    }
    if (eh_out.common.abi != 0) {
        fail(PICOLITE_ERROR_INCOMPATIBLE, "'" + settings.filename +"' has the wrong architecture");
    }
    if (eh_out.flags & EF_ARM_ABI_FLOAT_HARD) {
        fail(PICOLITE_ERROR_INCOMPATIBLE, "'" + settings.filename  +"' has the wrong architecture (hard float)");
    }
}

static void __noreturn fail_read_error() {
    fail(PICOLITE_ERROR_READ_FAILED, "Failed to read input file");
}
...
bool find_binary_info(memory_access& access, binary_info_header &hdr) {
    ...
    if (!base) {
        fail(PICOLITE_ERROR_FORMAT, "UF2 file does not contain a valid RP2 executable image");
    }
...
FILE *get_file(const char *mode) {
    ...
    if (!file) fail(PICOLITE_ERROR_READ_FAILED, "Could not open '%s'", settings.filename.c_str());
    return file;
}
...
void build_rmap_elf(FILE *file, range_map<size_t>& rmap) {
    ...
    if (eh.ph_entry_size != sizeof(elf32_ph_entry)) {
        fail(PICOLITE_ERROR_FORMAT, "Invalid ELF32 program header");
    }
    if (eh.ph_num) {
...
void build_rmap_uf2(FILE *file, range_map<size_t>& rmap) {
    ...
    do {
        if (1 != fread(&block, sizeof(uf2_block), 1, file)) {
            if (feof(file)) break;
            fail(PICOLITE_ERROR_READ_FAILED, "unexpected end of input file");
        }
...
static picoboot::connection get_single_bootsel_device_connection(device_map& devices, bool exclusive = true) {
    ...
    if (!rc) fail(PICOLITE_ERROR_USB, "Unable to connect to device");
...
bool load_command::execute(device_map &devices) {
    if (settings.offset_set && get_file_type() != filetype::bin) {
        fail(PICOLITE_ERROR_ARGS, "Offset only valid for BIN files");
    }
    ...
    for (auto mem_range : ranges) {
        ...
        if (t1 != t2 || t1 == invalid || t1 == rom) {
            fail(PICOLITE_ERROR_FORMAT, "File to load contained an invalid memory range 0x%08x-0x%08x", mem_range.from,
                 mem_range.to);
        }
        if (settings.load.no_overwrite && mem_range.intersects(flash_binary_range)) {
            if (flash_binary_end_unknown) {
                if (!settings.load.no_overwrite_force) {
                    fail(PICOLITE_ERROR_NOT_POSSIBLE, "-n option specified, but the size/presence of an existing flash binary could not be detected; aborting. Consider using the -N option");
                }
            } else {
                fail(PICOLITE_ERROR_NOT_POSSIBLE, "-n option specified, and the loaded data range clashes with the existing flash binary range %08x->%08x",
    ...
    for (auto mem_range : ranges) {
        ...
        if (settings.load.verify) {
            ...
            if (ok) {
                ...
                std::cout << "  FAILED\n";
                fail(PICOLITE_ERROR_VERIFICATION_FAILED, "The device contents did not match the file");
            }
        }
    }
    if (settings.load.execute) {
        ...
        if (!start) {
            fail(PICOLITE_ERROR_FORMAT, "Cannot execute as file does not contain a valid RP2 executable image");
        }
...
static int reboot_device(libusb_device *device, bool bootsel, uint disable_mask=0) {
    ...
    if (ret) {
        fail(PICOLITE_ERROR_USB, "Failed to get descriptor %d\n", ret);
    }
    libusb_device_handle *dev_handle;
    ret = libusb_open(device, &dev_handle);
    if (ret) {
    #if _WIN32
        fail(PICOLITE_ERROR_USB, "Unable to access device to reboot it; Make sure there is a driver installed via Zadig\n", ret);
    #else
        fail(PICOLITE_ERROR_USB, "Unable to access device to reboot it; Use sudo or setup a udev rule\n", ret);
    #endif
    }
    for (int i = 0; i < config->bNumInterfaces; i++) {
        if (0xff == config->interface[i].altsetting[0].bInterfaceClass &&
            ...
            if (ret) {
                fail(PICOLITE_ERROR_USB, "Failed to claim interface\n");
            }
            if (bootsel) {
            ...
            // if (ret != 0 ) {
            //     fail(PICOLITE_ERROR_UNKNOWN, "Unable to reset the device %d\n", ret);
            // }
            return 0;
            // return ret;
        }
    }
    fail(PICOLITE_ERROR_USB, "Unable to locate reset interface on the device");
}
...
int picolite_load(const char *filePath) {
    char *argv[6];
    argv[0] = const_cast<char*>("picolite");
    argv[1] = const_cast<char*>("load");
    argv[2] = const_cast<char*>("-u");
    argv[3] = const_cast<char*>("-x");
    argv[4] = const_cast<char*>(filePath);
    argv[5] = const_cast<char*>("-f");
    ...
    try {
        ...
        if (settings.reboot_usb && settings.reboot_app_specified) {
            fail(PICOLITE_ERROR_ARGS, "Cannot specify both -u and -a reboot options");
        }

        if (selected_cmd->get_device_support() != cmd::none) {
            if (libusb_init(&ctx)) {
                fail(PICOLITE_ERROR_USB, "Failed to initialise libUSB\n");
            }
        }
        ...
        for (int tries = 0; !rc && tries < 2; tries++) {
            if (ctx) {
                if (libusb_get_device_list(ctx, &devs) < 0) {
                    fail(PICOLITE_ERROR_USB, "Failed to enumerate USB devices\n");
                }
                    ...
                    if (devices[dr_vidpid_bootrom_ok].empty() &&
                        ...
                        auto printer = [&](enum picoboot_device_result r, const string &description) {
                            ...
                            printer(dr_vidpid_bootrom_cant_connect,
                                " appears to be a RP2040 device in BOOTSEL mode, but picolite was unable to connect. Maybe try 'sudo' or check your permissions.");
                        #else
                            printer(dr_vidpid_bootrom_cant_connect,
                                " appears to be a RP2040 device in BOOTSEL mode, but picolite was unable to connect. You may need to install a driver. See \"Getting started with Raspberry Pi Pico\" for more information");
                        #endif
                        ...
                        if (selected_cmd->force_requires_pre_reboot()) {
                                ...
                                printer(dr_vidpid_stdio_usb,
                                    " appears to be a RP2040 device with a USB serial connection, not in BOOTSEL mode. You can force reboot into BOOTSEL mode via 'picolite reboot -f -u' first.");
                            #else
                                printer(dr_vidpid_stdio_usb,
                        ...
                        if (!devices[dr_vidpid_bootrom_cant_connect].empty()) {
                            #if defined(__linux__) || defined(__APPLE__)
                                rc = PICOLITE_ERROR_PERMISSIONS;
                            #else
                                rc = PICOLITE_ERROR_DRIVER;
                            #endif
                        } else if (!devices[dr_vidpid_picoprobe].empty()) {
                            rc = PICOLITE_ERROR_BOOTSEL;
                        } else if (!devices[dr_vidpid_micropython].empty()) {
                            rc = PICOLITE_ERROR_BOOTSEL;
                        } else if (!devices[dr_vidpid_stdio_usb].empty()) {
                            rc = PICOLITE_ERROR_BOOTSEL;
                        } else {
                            rc = PICOLITE_ERROR_NO_DEVICE;
                        }
                        ...
                        if (devices[dr_vidpid_bootrom_ok].size() > 1 ||
                            ...
                            fail(PICOLITE_ERROR_NOT_POSSIBLE, "Command requires a single RP2040 device to be targeted.");
                        }
                        if (!devices[dr_vidpid_bootrom_ok].empty()) {
            ...
            if (!rc) {
                if (settings.force && ctx) { // actually ctx should never be null as we are targeting device if force is set, but still
                    if (devices[dr_vidpid_stdio_usb].size() != 1) {
                        fail(PICOLITE_ERROR_NOT_POSSIBLE,
                ...
                if (!selected_cmd->execute(devices) && tries) {
                    if (settings.force_no_reboot) {
                        fos << "\nThe device has been left accessible, but without the drive mounted; use 'picolite reboot' to reboot into regular BOOTSEL mode or application mode.\n";
                    } else {
