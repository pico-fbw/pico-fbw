...
#define PINS_NOT_EQUAL(p1, p2) ((p1) != (p2))

#if !PINS_NOT_EQUAL(INPUT_AIL_PIN, SERVO_AIL_PIN) || \
    !PINS_NOT_EQUAL(INPUT_ELEV_PIN, SERVO_ELEV_PIN) || \
    !PINS_NOT_EQUAL(INPUT_RUD_PIN, SERVO_RUD_PIN) || \
    !PINS_NOT_EQUAL(INPUT_AIL_PIN, MODE_SWITCH_PIN) || \
    !PINS_NOT_EQUAL(INPUT_AIL_PIN, IMU_SDA_PIN) || \
    !PINS_NOT_EQUAL(INPUT_AIL_PIN, IMU_SCL_PIN) || \
    !PINS_NOT_EQUAL(INPUT_AIL_PIN, GPS_TX_PIN) || \
    !PINS_NOT_EQUAL(INPUT_AIL_PIN, GPS_RX_PIN) || \
    !PINS_NOT_EQUAL(SERVO_AIL_PIN, MODE_SWITCH_PIN) || \
    !PINS_NOT_EQUAL(SERVO_AIL_PIN, IMU_SDA_PIN) || \
    !PINS_NOT_EQUAL(SERVO_AIL_PIN, IMU_SCL_PIN) || \
    !PINS_NOT_EQUAL(SERVO_AIL_PIN, GPS_TX_PIN) || \
    !PINS_NOT_EQUAL(SERVO_AIL_PIN, GPS_RX_PIN) || \
    !PINS_NOT_EQUAL(INPUT_ELEV_PIN, MODE_SWITCH_PIN) || \
    !PINS_NOT_EQUAL(INPUT_ELEV_PIN, IMU_SDA_PIN) || \
    !PINS_NOT_EQUAL(INPUT_ELEV_PIN, IMU_SCL_PIN) || \
    !PINS_NOT_EQUAL(INPUT_ELEV_PIN, GPS_TX_PIN) || \
    !PINS_NOT_EQUAL(INPUT_ELEV_PIN, GPS_RX_PIN) || \
    !PINS_NOT_EQUAL(SERVO_ELEV_PIN, MODE_SWITCH_PIN) || \
    !PINS_NOT_EQUAL(SERVO_ELEV_PIN, IMU_SDA_PIN) || \
    !PINS_NOT_EQUAL(SERVO_ELEV_PIN, IMU_SCL_PIN) || \
    !PINS_NOT_EQUAL(SERVO_ELEV_PIN, GPS_TX_PIN) || \
    !PINS_NOT_EQUAL(SERVO_ELEV_PIN, GPS_RX_PIN) || \
    !PINS_NOT_EQUAL(INPUT_RUD_PIN, MODE_SWITCH_PIN) || \
    !PINS_NOT_EQUAL(INPUT_RUD_PIN, IMU_SDA_PIN) || \
    !PINS_NOT_EQUAL(INPUT_RUD_PIN, IMU_SCL_PIN) || \
    !PINS_NOT_EQUAL(INPUT_RUD_PIN, GPS_TX_PIN) || \
    !PINS_NOT_EQUAL(INPUT_RUD_PIN, GPS_RX_PIN) || \
    !PINS_NOT_EQUAL(SERVO_RUD_PIN, MODE_SWITCH_PIN) || \
    !PINS_NOT_EQUAL(SERVO_RUD_PIN, IMU_SDA_PIN) || \
    !PINS_NOT_EQUAL(SERVO_RUD_PIN, IMU_SCL_PIN) || \
    !PINS_NOT_EQUAL(SERVO_RUD_PIN, GPS_TX_PIN) || \
    !PINS_NOT_EQUAL(SERVO_RUD_PIN, GPS_RX_PIN) || \
    !PINS_NOT_EQUAL(MODE_SWITCH_PIN, IMU_SDA_PIN) || \
    !PINS_NOT_EQUAL(MODE_SWITCH_PIN, IMU_SCL_PIN) || \
    !PINS_NOT_EQUAL(MODE_SWITCH_PIN, GPS_TX_PIN) || \
    !PINS_NOT_EQUAL(MODE_SWITCH_PIN, GPS_RX_PIN) || \
    !PINS_NOT_EQUAL(IMU_SDA_PIN, IMU_SCL_PIN) || \
    !PINS_NOT_EQUAL(IMU_SDA_PIN, GPS_TX_PIN) || \
    !PINS_NOT_EQUAL(IMU_SDA_PIN, GPS_RX_PIN) || \
    !PINS_NOT_EQUAL(IMU_SCL_PIN, GPS_TX_PIN) || \
    !PINS_NOT_EQUAL(IMU_SCL_PIN, GPS_RX_PIN) || \
    !PINS_NOT_EQUAL(GPS_TX_PIN, GPS_RX_PIN)
