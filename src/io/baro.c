...
int baro_init() {
    baroModel = (BaroModel)flash.sensors[SENSORS_BARO_MODEL];
    if (platform_is_fbw()) {
        baroModel = BARO_MODEL_DPS310;
    }
    if (print.fbw) printf("[baro] searching for ");
    switch (baroModel) {
    ...
    if (result == 1) { // Bytes written should be 1
        ...
        if (result == 1) {
            if (print.imu) printf("[baro] ID: 0x%02X\n", id);
            if (id == CHIP_ID) {
                if (print.imu) printf("[baro] ID ok\n");
                result = 0;
                baroUsable = true;
            } else {
                if (print.imu) printf("[baro] ERROR: ID does not match expected value\n");
                result = 1;
            }
        } else {
            if (print.fbw) printf("[baro] ERROR: unable to read ID\n");
        }
    } else {
        if (print.fbw) printf("[baro] ERROR: address not acknowledged (no/wrong device present?)\n");
    }
    return result;
}
