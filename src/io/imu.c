 ...
 * @return Number of bytes read (should be 22), or PICO_ERROR_GENERIC if address not acknowledged, no device present.
 * PICO_ERROR_TIMEOUT if a timeout occured.
...
int imu_init() {
    ...
    imuModel = config.sensors.imuModel;
    if (platform_is_fbw()) {
    ...
    if (result == 1) { // 1 byte should be written
        ...
        if (result == 1) {
            ...
            if (id == CHIP_ID) {
                if (config.debug.debug_imu) printf("[imu] ID ok\n");
                result = 0;
            } else {
            ...
            if (config.debug.debug_fbw) printf("[imu] ERROR: unable to read ID\n");
        }
    } else {
        if (config.debug.debug_fbw) printf("[imu] ERROR: address not acknowledged (no/wrong device present?)\n");
    }
    return result;
}
...
bool imu_configure() {
    if (config.debug.debug_fbw) printf("[imu] configuring sensor\n");
    switch (imuModel) {
        case IMU_MODEL_BNO055:
            ...
            if (imu_isCalibrated()) {
                if ((IMUModel)flash_readFloat(FLOAT_SECTOR_IMU_MAP, 1) != IMU_MODEL_BNO055) {
                    if (config.debug.debug_fbw) printf("[imu] WARNING: calibration was completed for a different IMU! rebooting to re-calibrate now...\n");
                ...
                while (calibrationSYS < 3) {
                    bno_getCalibrationStatus(&calibrationSYS, &calibrationGYR, &calibrationACC, &calibrationMAG);
                    if (config.debug.debug_fbw) printf("All: %d/3, Gyr: %d/3, Acc: %d/3, Mag: %d/3\n", calibrationSYS, calibrationGYR, calibrationACC, calibrationMAG);
                    sleep_ms(1000);
                }
            } else {
...
Euler imu_getRawAngles() {
    ...
    i2cErr:
        if (config.debug.debug_imu) printf("[imu] ERROR: i2c read error occured!\n");
        setIMUSafe(false);
        return (Euler){0};
}
...
bool imu_calibrate() {
    switch (imuModel) {
        case IMU_MODEL_BNO055: {
            ...
            if (config.debug.debug_fbw) printf("[imu] starting imu axis mapping calibration\n");
            ...
            while (currentAxis < EULER_AXIS_NONE) {
                ...
                if (config.debug.debug_imu) printf("[imu] moved axis was %d @ %ddeg (reverse: %d)\n", movedAxis, movedAngle, calibration[currentAxis + 3]);
            ...
            if (config.debug.debug_imu) printf("[imu] final map is 0x%02X, final sign is 0x%02X\n", map, sign);
            // Write to flash
        ...
        default: {
            if (config.debug.debug_fbw) printf("[imu] ERROR: unable to calibrate this model!\n");
            return false;
        }
    }
    return true;
