...
i32 api_set_target(const char *cmd, const char *args) {
    if (aircraft.mode == MODE_NORMAL) {
        ...
        if (fabsf(roll) > config.control[CONTROL_ROLL_LIMIT_HOLD] ||
            pitch > config.control[CONTROL_PITCH_UPPER_LIMIT] ||
            pitch < config.control[CONTROL_PITCH_LOWER_LIMIT] ||
            fabsf(yaw) > config.control[CONTROL_MAX_RUD_DEFLECTION])
            return 400;
